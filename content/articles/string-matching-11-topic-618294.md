---
title: 后缀排序比较：字符串匹配训练题解
category: 字符串匹配
summary: 围绕“后缀排序比较”整理的训练型题解：用专属图解拆出状态、不变量和代码落点。
problem_ids: [1316, 28, 459, 214, 5, 647]
order: 111
---

# 后缀排序比较：字符串匹配训练题解

这篇不是背模板，而是把 **后缀排序比较** 拆成可以手写、可以检查的步骤。训练时建议先遮住题解，只看图和不变量，自己写一版，再展开代码对照。
...
建议按这个顺序刷：#1316, #28, #459, #214, #5, #647。每题都先写 Go 或 Rust，再对照题解。
