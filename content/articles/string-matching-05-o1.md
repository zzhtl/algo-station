---
title: 字符串哈希 O(1) 比较：字符串匹配训练题解
category: 字符串匹配
summary: 围绕“字符串哈希 O(1) 比较”整理的训练型题解：用专属图解拆出状态、不变量和代码落点。
problem_ids: [686, 796, 1392, 187, 1044, 3008]
order: 105
---

# 字符串哈希 O(1) 比较：字符串匹配训练题解

这篇不是背模板，而是把 **字符串哈希 O(1) 比较** 拆成可以手写、可以检查的步骤。训练时建议先遮住题解，只看图和不变量，自己写一版，再展开代码对照。
...
建议按这个顺序刷：#686, #796, #1392, #187, #1044, #3008。每题都先写 Go 或 Rust，再对照题解。
