---
title: Z 函数求每位扩展：字符串匹配训练题解
category: 字符串匹配
summary: 围绕“Z 函数求每位扩展”整理的训练型题解：用专属图解拆出状态、不变量和代码落点。
problem_ids: [5, 647, 686, 796, 1392, 187]
order: 103
---

# Z 函数求每位扩展：字符串匹配训练题解

这篇不是背模板，而是把 **Z 函数求每位扩展** 拆成可以手写、可以检查的步骤。训练时建议先遮住题解，只看图和不变量，自己写一版，再展开代码对照。
...
建议按这个顺序刷：#5, #647, #686, #796, #1392, #187。每题都先写 Go 或 Rust，再对照题解。
